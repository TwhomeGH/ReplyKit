...
kernel void rotateNV12_bilinear(
    ...
    float centerFix = 0.5f * (uniformScale - 1.0f);

    float srcXf = ((float(gid.x) + 0.5f - offsetX) - centerFix) / uniformScale;
    float srcYf = ((float(gid.y) + 0.5f - offsetY) - centerFix) / uniformScale;
