...
case 2: videoSettings.bitRateMode = .variable  // 需 iOS 26+
...
- VBR 在 HaishinKit 中標記為 `@available(iOS 26.0, *)`（2025/10 才加入），不是 Apple 限制，而是 library 作者刻意設的 placeholder
- Apple VideoToolbox 的 `kVTCompressionPropertyKey_VariableBitRate` 從 iOS 13+ 就已存在
